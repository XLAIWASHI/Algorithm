...
快速排序每次 `partition` 都需要扫描当前子数组中的元素，因此对长度为 k 的子数组进行一次 `partition` ，时间复杂度为 $O(k)$ 

> 在同一递归层中，所有子数组的总长度为 n，因此该层 `partition` 的总复杂度为 $O(n)$ 

---
**最好情况** 

每次 `partition` 都把数组分得尽可能平均
